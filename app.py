...
def fix_date(row):
    ...
    if '/' not in val:
        ...
        if ts is not None and not pd.isna(ts) and ts.month != 1:
            return pd.Timestamp(f'2026-01-{ts.month:02d}')
        return ts
    else:
...
def load_2026_daily():
    ...
    df = df[pd.to_numeric(df['الرقم'], errors='coerce').notna()]
    df['الرقم'] = pd.to_numeric(df['الرقم'], errors='coerce')
    df = df[df['الرقم'] > 0]
    df['التاريخ'] = df.apply(fix_date, axis=1)
    df = df[df['التاريخ'].notna()]
