...
def dashboard():
    ...
    branch_summary = (
        filtered.groupby('branch_id', as_index=False)
                .agg(المبيعات=('total_sales', 'sum'),
                     الارباح=('total_profit', 'sum'))
    )
    branch_summary['اسم الفرع'] = 'فرع ' + branch_summary['branch_id'].astype(str)
    branch_summary['نسبة الربح'] = (branch_summary['الارباح'] / branch_summary['المبيعات'] * 100).round(1)
    ...
    if not branch_summary.empty:
        ...
        best_profit_branch = branch_summary.loc[branch_summary['الارباح'].idxmax(), 'اسم الفرع']
        best_profit_val = branch_summary['الارباح'].max()
        best_margin_branch = branch_summary.loc[branch_summary['نسبة الربح'].idxmax(), 'اسم الفرع']
        ...
        <div class="analysis">
            <div class="analysis-card">
                ...
                <div class="analysis-text">
                    <strong>أعلى مبيعات</strong>
                    <span class="analysis-branch">{best_sales_branch}</span>
            ...
            <div class="analysis-card">
                <div class="analysis-icon">💰</div>
                <div class="analysis-text">
                    <strong>أعلى أرباح</strong>
                    <span class="analysis-branch">{best_profit_branch}</span>
            ...
            <div class="analysis-card">
                <div class="analysis-icon">📊</div>
                <div class="analysis-text">
                    <strong>أعلى نسبة ربح من المبيعات</strong>
                    <span class="analysis-branch">{best_margin_branch}</span>
                    <span class="analysis-val">{best_margin_val}%</span>
                </div>
            </div>
        </div>
        '''
    ...
    fig.add_trace(go.Bar(
        name='الأرباح',
        x=branch_summary['اسم الفرع'],
        y=branch_summary['الارباح'],
        marker_color='#2ecc71',
        text=branch_summary['الارباح'].apply(lambda x: f'{x:,.0f}'),
        textposition='outside'
    ))
    fig.update_layout(
