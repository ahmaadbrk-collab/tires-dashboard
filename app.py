...
invoice_level['date_only'] = invoice_level['date'].dt.date
...
def dashboard():
    ...
    branch_summary['branch_name'] = 'فرع ' + branch_summary['branch_id'].astype(str)

    fig_branch = px.bar(
        branch_summary, x='branch_name', y=['total_sales', 'total_profit'],
        barmode='group', title='مقارنة المبيعات والأرباح بين الفروع',
        labels={'value': 'المبلغ', 'branch_name': 'الفرع', 'variable': 'النوع'},
        color_discrete_map={'total_sales': '#3498db', 'total_profit': '#2ecc71'}
    )
    ...
    daily['branch_name'] = 'فرع ' + daily['branch_id'].astype(str)

    fig_trend = px.line(
        daily, x='date_only', y='total_sales', color='branch_name',
        title='المبيعات اليومية لكل فرع',
        labels={'date_only': 'التاريخ', 'total_sales': 'المبيعات', 'branch_name': 'الفرع'}
    )
    ...
    <head>
        ...
        <style>
            ...
            .filters {{ background: white; padding: 15px; border-radius: 10px; margin-bottom: 20px; display: flex; gap: 20px; align-items: center; flex-wrap: wrap; }}
            .filters label {{ font-weight: bold; }}
            .filters select, .filters input {{ padding: 8px; border-radius: 5px; border: 1px solid #ccc; font-size: 14px; }}
            .filters button {{ padding: 8px 20px; background: #3498db; color: white; border: none; border-radius: 5px; cursor: pointer; font-size: 14px; }}
            .kpi {{ display: flex; justify-content: space-around; margin-bottom: 20px; flex-wrap: wrap; }}
            .kpi-card {{ background: white; padding: 20px 30px; border-radius: 10px; text-align: center; flex: 1; margin: 10px; min-width: 150px; box-shadow: 0 2px 5px rgba(0,0,0,0.1); }}
            .kpi-value {{ font-size: 28px; font-weight: bold; color: #3498db; }}
            .kpi-value.green {{ color: #2ecc71; }}
            .chart {{ background: white; border-radius: 10px; padding: 10px; margin-bottom: 20px; box-shadow: 0 2px 5px rgba(0,0,0,0.1); }}
        </style>
    </head>
    <body>
