...
# نسب الربح الشهرية لكل فرع من تقارير الأرباح
PROFIT_RATES = {
    ('1','2026-01'): 0.2879,
    ('1','2026-02'): 0.3413,
    ('2','2026-01'): 0.4195,
...
def load_2026_daily():
    ...
    df.columns = ['الرقم','التاريخ','ف1_كمية','ف1_مبيعات','ف2_كمية','ف2_مبيعات','ف3_كمية','ف3_مبيعات','ف4_كمية','ف4_مبيعات','إجمالي_كمية','إجمالي_مبيعات','رصيد_كمية','رصيد_مبيعات']
    df = df[df['التاريخ'].notna()]
    df['التاريخ'] = pd.to_datetime(df['التاريخ'], errors='coerce')
    df = df[df['التاريخ'].notna()]
    df = df[df['التاريخ'].dt.year == 2026]
    for col in ['ف1_مبيعات','ف2_مبيعات','ف3_مبيعات','ف4_مبيعات']:
    ...
    for _, row in df.iterrows():
        ...
        for branch, col in [('1','ف1_مبيعات'),('2','ف2_مبيعات'),('3','ف3_مبيعات'),('4','ف4_مبيعات')]:
            sales = row[col]
...
monthly26 = df26_daily.groupby(['branch_id','Month','BranchName']).agg(
    sales=('TotalSales','sum'), cost=('TotalCost','sum'),
    profit=('TotalProfit','sum'), invoices=('invoices','sum')
).reset_index()
...
# بيانات يومية كاملة (2025 + 2026)
daily25 = df25.groupby(['branch_id','BranchName','SaleDate']).agg(
    sales=('TotalSales','sum'), profit=('TotalProfit','sum')
).reset_index()
daily25['Month'] = daily25['SaleDate'].dt.strftime('%Y-%m')

daily26 = df26_daily.groupby(['branch_id','BranchName','SaleDate']).agg(
    sales=('TotalSales','sum'), profit=('TotalProfit','sum')
).reset_index()
...
def dashboard():
    ...
    # مخطط يومي للشهر المحدد أو شهري للكل
    fd = all_daily.copy()
    if branch_filter != 'all':
        fd = fd[fd['branch_id'] == branch_filter]
    if month_filter != 'all':
        fd = fd[fd['Month'] == month_filter]
        daily_agg = fd.groupby('SaleDate').agg(sales=('sales','sum'), profit=('profit','sum')).reset_index()
        fig2 = go.Figure()
        ...
        fig2.update_layout(title=f'المبيعات اليومية - {month_filter}', barmode='group',
            font=dict(family='Arial'), dragmode=False, hovermode=False, margin=dict(t=60,b=40))
    else:
